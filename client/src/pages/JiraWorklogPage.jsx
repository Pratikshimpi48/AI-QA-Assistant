...
export default function JiraWorklogPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (res?.worklog) {
        ...
        showToast('✨ Work log generated successfully!')
      }
    } catch (err) {
      setErrorMsg(err.response?.data?.message || 'Failed to generate work log.')
    } finally {
      setGenerating(false)
    }
  }
  ...
  const handleCopyClipboard = (text) => {
    ...
    showToast('📋 Copied to clipboard! You can paste this directly into Jira.')
  }
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: '#f8fafc', margin: 0, letterSpacing: '-0.02em' }}>
              AI Work Log Generator
            </h1>
            <p style={{ fontSize: '0.875rem', color: '#94a3b8', margin: 0 }}>
              Retrieve Jira ticket details, generate professional QA updates, and review before logging time.
            </p>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: jiraConnected ? '#4ade80' : '#fbbf24' }}>
              {jiraConnected
                ...
                : 'Jira is not connected. Connect in Settings to auto-fetch live ticket summaries and comments.'}
            </span>
          </div>
          {!jiraConnected && (
            <Link
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.1fr) minmax(0, 0.9fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
        ...
        <div style={{
          ...
          {/* Additional User Work Notes */}
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#94a3b8', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              📝 Additional Work Notes / Testing Performed (Optional)
            </label>
            <textarea
              rows={3}
              value={userNotes}
              onChange={e => setUserNotes(e.target.value)}
              placeholder="e.g. Verified positive checkout flow, tested boundary values for payload, found minor CSS spacing issue."
              style={{
          ...
          >
            {generating ? (
              <span>✨ Generating Work Log with AI...</span>
            ) : (
              <span>✨ Generate Work Log with AI</span>
            )}
          </button>
        </div>

        {/* Right Column: Ticket Context Preview Card */}
        <div style={{
          background: 'rgba(15, 23, 42, 0.75)', border: '1px solid rgba(255, 255, 255, 0.08)',
          borderRadius: '1rem', padding: '1.5rem', backdropFilter: 'blur(12px)',
          ...
          <div>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#f8fafc', margin: '0 0 1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>📋 Live Jira Ticket Preview</span>
            </h2>
            ...
            {ticketDetails ? (
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                  <span style={{ padding: '0.15rem 0.5rem', borderRadius: '0.375rem', background: 'rgba(255,255,255,0.05)', fontSize: '0.7rem', color: '#94a3b8' }}>
                    Type: {ticketDetails.issueType}
                  </span>
                  <span style={{ padding: '0.15rem 0.5rem', borderRadius: '0.375rem', background: 'rgba(255,255,255,0.05)', fontSize: '0.7rem', color: '#94a3b8' }}>
                ...
                {ticketDetails.descriptionText && (
                  <div style={{ marginBottom: '1rem' }}>
                    <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>DESCRIPTION</span>
                    <p style={{
                      fontSize: '0.78rem', color: '#cbd5e1', background: 'rgba(0,0,0,0.2)',
                      padding: '0.65rem 0.85rem', borderRadius: '0.5rem', margin: 0,
                ...
                {ticketDetails.commentsText && (
                  <div>
                    <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>RECENT COMMENTS</span>
                    <pre style={{
                      fontSize: '0.75rem', color: '#94a3b8', background: 'rgba(0,0,0,0.2)',
                      padding: '0.65rem 0.85rem', borderRadius: '0.5rem', margin: 0,
                      maxHeight: '110px', overflowY: 'auto', whiteSpace: 'pre-wrap', fontFamily: 'monospace',
                    }}>
                      {ticketDetails.commentsText}
                    </pre>
                  </div>
              ...
              <div style={{ padding: '3rem 1rem', textAlign: 'center', color: '#64748b' }}>
                ...
                <div style={{ fontSize: '0.9rem', fontWeight: 600, color: '#94a3b8' }}>No ticket loaded</div>
                <p style={{ fontSize: '0.78rem', color: '#475569', marginTop: '0.25rem', lineHeight: 1.5 }}>
                  Select a ticket from your Watchlist or type a Jira Ticket ID and click "Fetch Details" to inspect live Jira context.
                </p>
              </div>
            )}
          </div>
          ...
          <div style={{
            ...
            🔒 <strong>Read-Only Jira Safety</strong>: All ticket data is fetched via read-only GET calls. Work logs are saved locally and copied to your clipboard.
          </div>
        </div>
      </div>

      {/* Interactive Review & Edit Work Log Section */}
      {generatedWorklog && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#f8fafc', margin: 0 }}>
                Review & Edit Work Log — [{generatedWorklog.ticketId}]
              </h2>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#38bdf8', marginBottom: '0.4rem', textTransform: 'uppercase' }}>
                Formatted Jira Work Log Comment (Ready to Paste)
              </label>
              <textarea
                rows={10}
        ...
        {savedLogs.length === 0 ? (
          <div style={{ padding: '2.5rem 1rem', textAlign: 'center', color: '#64748b' }}>
            ...
            <p style={{ fontSize: '0.75rem', color: '#475569', marginTop: '0.25rem' }}>
              Generated work logs that you save will be listed here for reference.
            </p>
          </div>
        ) : (
